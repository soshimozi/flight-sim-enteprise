...
public class AircraftMaintenanceBean implements Serializable
{
	...
	class EngineData
	{
		...
		public String report(boolean printNumber, int number)
		{
			...
			if (printNumber)
				report.append("<tr><td class=\"oneunderline\">Engine ").append(number).append("</td></tr>\n");
            else
				report.append("<tr><td class=\"oneunderline\">Engine information</td></tr>\n");

			report.append("<tr><td class=\"one\">Engine running time: ").append(logEngineTime / 3600).append(" hours.</td></tr>");
			if (cylinderCleaning > 0)
			{
				int hours = cylinderCleaning/3600;
				String severity = hours > 40 ? "serious" : hours > 20 ? "moderate" : "low"; 
				report.append("<tr><td class=\"one\">A ").append(severity).append(" amount of carbon deposits was detected and removed from the engine cylinders and ").append("exhaust valves.</td><td class=\"two\">").append(Formatters.currency.format(Math.round(marginFactor() * cylinderCleaning / 360))).append("</td></tr>\n");
			}
            else
            {
			...
			if (worstCompression > 0)
				report.append("<tr><td class=\"one\">Compression test shows the worst cylinder at ").append(worstCompression).append("% compression.</td></tr>\n");
	...
	void loadEngineData(ResultSet rs) throws SQLException
	{
		...
		{
			int engine = rs.getInt("engine");
			if (engine <= 0 || engine > engineData.length)
				continue;
	...
	int doMaintenance(ResultSet damageRs, ResultSet maintenanceRs, int log) throws SQLException
	{
        ...
        for (EngineData anEngineData : engineData)
            totalCost += anEngineData.maintenance();
	...
	public String report()
	{
		StringBuilder report = new StringBuilder();
		report.append("<table>");
		report.append("<tr><td class=\"one\">Aircraft Serviced: ").append(aircraft.getRegistration()).append("</td><td class=\"two\"></td></tr>");
		report.append("<tr><td class=\"oneunderline\">Maintenance Handled By: ").append(fbo.getName()).append("</td><td class=\"two\"></td></tr>");
		
		if (type == MAINT_FIXAIRCRAFT)
			report.append("<tr><td class=\"one\">").append(maintenanceType(type)).append("</td><td class=\"two\">").append(Formatters.currency.format(totalPrice)).append("</td></tr>");
		else if(type == MAINT_SHIPMENTDISASSEMBLY)
			report.append("<tr><td class=\"one\">Aircraft Disassembly for Shipment </td><td class=\"two\">").append(Formatters.currency.format(totalPrice)).append("</td></tr>");
		else if(type == MAINT_SHIPMENTREASSEMBLY)
			report.append("<tr><td class=\"one\">Aircraft Reassembly for Shipment </td><td class=\"two\">").append(Formatters.currency.format(totalPrice)).append("</td></tr>");
		else
			report.append("<tr><td class=\"one\">").append(maintenanceType(type)).append("</td><td class=\"two\">").append(Formatters.currency.format(marginFactor() * aircraft.getRawMaintenancePrice(type))).append("</td></tr>");
		
		if (type == MAINT_100HOUR)
		{
			report.append("<tr><td class=\"one\">Additional Engine Repair </td><td class=\"two\">").append(Formatters.currency.format(Math.round(marginFactor() * ageECost))).append("</td></tr>");
			report.append("<tr><td class=\"one\">Avionics inspection & repair </td><td class=\"two\">").append(Formatters.currency.format(Math.round(marginFactor() * ageAvCost))).append("</td></tr>");
			report.append("<tr><td class=\"one\">Airframe inspection & repair </td><td class=\"two\">").append(Formatters.currency.format(Math.round(marginFactor() * ageAfCost))).append("</td></tr>");
			report.append("<tr><td class=\"one\">Airworthiness Directives Compliance </td><td class=\"two\">").append(Formatters.currency.format(Math.round(marginFactor() * ageAdwCost))).append("</td></tr>");
		...
		report.append("</table>");
		report.append("<div id=\"total\">").append(Formatters.currency.format(totalPrice)).append("</div>");
	...
	public static String maintenanceType(int type)
	{
		switch(type)
		{
			...
			case AircraftMaintenanceBean.MAINT_SHIPMENTDISASSEMBLY:
