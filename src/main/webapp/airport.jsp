...
<%
	...
	HashSet<String> aircraftList = new HashSet<String>();
%>
...
%>
	<div class="infoBlock">
	<table>
			<tr><th colspan="3">Close airports</th></tr>
<%
...
%>
			<tr><td><a href="<%= response.encodeURL(URL) %>"><%= icao %></a></td><td><%= value %> nm</td><td><img src="<%= image %>" /></td></tr>
<%
			}
		}
%>
...
%>
	...
	<table>
		<tr valign="top">
			<td valign="top"><table><tr><td><h1><jsp:getProperty name="airport" property="icao" /></h1></td><td><a href="#" onclick="gmap.setSize(620,520);gmap.setUrl('gmap.jsp?icao=<%= airport.getIcao() %>');gmap.showPopup('gmap');return false;" id="gmap"><img style="border-style: none;" src="<%= airport.getDescriptiveImage(fbo) %>" /></a>
			</td><td><a href="http://www.fscharts.com/?action=search&type=icao&term=<%= airport.getIcao() %>" onclick="this.target='_blank'"><img style="border-style: none;" src="img/fscharts.gif"/></a></td>
			</tr></table>
			  <%= airport.getTitle() %><br/>			
...
%>
			...
			Lat: <%= latNS %>, Long: <%= lonEW %>, Elev: <%= elev %>
			<br/><br/>
			<table id="fboInfo">
<%
		...
		if (user.getLevel() == UserBean.LEV_MODERATOR)
			fuel = "<a title=\"Edit fuel price in this region\" href=\"" + response.encodeURL("editfuel.jsp?icao=" + airport.getIcao()) + "\">" + fuel + "</a>";
...
%>						<tr><td><b>FBO</b></td>
						...
						<td><b>Jet-A Fuel</b></td>			  	  
			  			<td><b>Gallons</b></td>	
			  			<td><b>Repair/Avionics</b></td></tr>
<%	
			if (airport.isAvgas() || hasRepairShop) 
			{ 
%>	  			<tr><td>Local market</td>			  	 
				<td><%= airport.isAvgas() ? fuel : "&nbsp;" %></td>			
				<td><%= airport.isAvgas() ? "unlimited" : "&nbsp;" %></td>
				<td><%= airport.isJetA() ? jeta : "&nbsp;" %></td>			
				<td><%= airport.isJetA() ? "unlimited" : "&nbsp;" %></td>
				<td><%= hasRepairShop ? "20% / 50%" : "" %></td></tr>
<%			}
			for (int c=0; c < fbo.length; c++) 
			{				
				GoodsBean fuelleft = data.getGoods(airport.getIcao(), fbo[c].getOwner(), GoodsBean.GOODS_FUEL100LL);
				GoodsBean jetaleft = data.getGoods(airport.getIcao(), fbo[c].getOwner(), GoodsBean.GOODS_FUELJETA);
				fuel = Formatters.currency.format(fbo[c].getFuel100LL());
				jeta = Formatters.currency.format(fbo[c].getFueljeta());
				UserBean fboowner = data.getAccountById(fbo[c].getOwner());
				String fboname = fbo[c].getName();
				boolean UserIsFBOStaff = fbo[c].updateAllowed(user);
				if (UserIsFBOStaff)
					fuel = "<a title=\"Edit FBO\" href=\"" + response.encodeURL("editfbo.jsp?id=" + fbo[c].getId()) + "\">" + fuel + "</a>";

				if (UserIsFBOStaff || fbo[c].logsVisibleToAll() ) 
					fboname = "<a title=\"View FBO Log\" href=\"" + response.encodeURL("fbolog.jsp?id=" + fbo[c].getId()) + "\">" + fboname + "</a>";
				
				if (fbo[c].isForSale())
					fboname = fboname + "<img src=\"img/sale.gif\" border=\"0\" title=\"" + Formatters.currency.format(fbo[c].getPrice()) + (fbo[c].getPriceIncludesGoods() ? " + goods" : "") + "\">";
					
				if (!fbo[c].getName().equalsIgnoreCase(fboowner.getName()))
					fboname = fboname + "<br><span class=\"small\"><i>" + fboowner.getName() + "</i></span>";
					
				if (UserIsFBOStaff) 
				{
					GoodsBean suppliesleft = data.getGoods(airport.getIcao(), fbo[c].getOwner(), GoodsBean.GOODS_SUPPLIES);
					int DaysSupplied = suppliesleft != null ? suppliesleft.getAmount() / fbo[c].getSuppliesPerDay(airport) : 0;
					if (DaysSupplied > 14) 
					{
...
%>				<tr><td><%= fboname %></td>
				<td><%= fuel %></td>
				<td><%= (int)Math.floor(fuelleft != null ? fuelleft.getAmount()/Data.GALLONS_TO_KG : 0) %></td>			  	  
				<td><%= jeta %></td>
				<td><%= (int)Math.floor(jetaleft != null ? jetaleft.getAmount()/Data.GALLONS_TO_KG : 0) %></td>
				<td><%= (fbo[c].getServices() & FboBean.FBO_REPAIRSHOP) > 0 ? fbo[c].getSServices() : "" %></td></tr>	
<%	 		}
			if (fboinactive.length > 0) 
			{
				...
				if (fbo.length > 0) 
				{
					...
					for (int c=0; c < fboinactive.length; c++)
					{
						if (fboinactive[c].isForSale()) 
						{
...
%>
					<tr><td class="disabledtext" colspan="4"><a href="javascript:showInactive()"><%= fboinactive.length %> closed <%= (fboinactive.length > 1 ? "FBOs" : "FBO") + inactiveForSaleFlag %></a></td></tr>
<%				}
				for (int c=0; c < fboinactive.length; c++) 
				{				
					GoodsBean fuelleft = data.getGoods(airport.getIcao(), fboinactive[c].getOwner(), GoodsBean.GOODS_FUEL100LL);
					GoodsBean jetaleft = data.getGoods(airport.getIcao(), fboinactive[c].getOwner(), GoodsBean.GOODS_FUELJETA);
					fuel = "Closed";
					jeta = "";
					UserBean fboowner = data.getAccountById(fboinactive[c].getOwner());
					String fboname = fboinactive[c].getName();
					boolean UserIsFBOStaff = fboinactive[c].updateAllowed(user);
					if (UserIsFBOStaff) 
						fuel = "<a title=\"Edit FBO\" href=\"" + response.encodeURL("editfbo.jsp?id=" + fboinactive[c].getId()) + "\">" + fuel + "</a>";
					
					if (UserIsFBOStaff || fboinactive[c].logsVisibleToAll() ) 
						fboname = "<a title=\"View FBO Log\" href=\"" + response.encodeURL("fbolog.jsp?id=" + fboinactive[c].getId()) + "\">" + fboname + "</a>";							
					
					if (fboinactive[c].isForSale())
						fboname = fboname + "<img src=\"img/sale.gif\" border=\"0\" title=\"" + Formatters.currency.format(fboinactive[c].getPrice()) + (fboinactive[c].getPriceIncludesGoods() ? " + goods" : "") + "\">";
						
					if (!fboinactive[c].getName().equalsIgnoreCase(fboowner.getName()))
						fboname = fboname + "<br><span class=\"small\"><i>" + fboowner.getName() + "</i></span>";
%>
					<tr class="InactiveFBO" <%= inactiveStyle %>><td class="disabledtext"><%= fboname %></td>
					<td class="disabledtext"><%= fuel %></td>
					<td class="disabledtext"><%= (int)Math.floor(fuelleft != null ? fuelleft.getAmount()/Data.GALLONS_TO_KG : 0) %></td>		
					<td class="disabledtext"><%= jeta %></td>
					<td class="disabledtext"><%= (int)Math.floor(jetaleft != null ? jetaleft.getAmount()/Data.GALLONS_TO_KG : 0) %></td>	  	  
					<td class="disabledtext">&nbsp;</td></tr>	
<%	 			}	
			}
		}
%>
...
<%
		...
		if (carriers.length > 0)
		{
			...
			Set<String> carrierSet = new HashSet<String>();
			for (int i = 0; i < carriers.length; i++)
			{
				if (!"".equals(carriers[i].getName().trim()))
					carrierSet.add(carriers[i].getName());
				if (carriers[i].getIsDefault())
				{
					FboBean parentFbo = data.getFbo(carriers[i].getFboId());
					if (data.calcFboFacilitySpaceAvailable(carriers[i], parentFbo, airport) > 0)
						spaceAvailable = true;
				}
			}
			String[] carrierNames = (String[]) carrierSet.toArray(new String[0]);
%>				
			<div>
			<b>Local Carriers:</b>
			<br />
...
<%				
			...
			if (spaceAvailable)
			{
%>				<br /><br /><a href="fbofacilityrent.jsp?icao=<%= airport.getIcao() %>">Space available</a>
<%			} 
%>			</div>
<%		}
%>		  </td>
...
<%		for (int c=0; c< staffGroups.length; c++)
		{ 
%>	 	     	<option class="formselect" value="<%= staffGroups[c].groupId%>"><%= staffGroups[c].groupName%></option>
<%		}
%>			</select>
			<br/>
...
<%		for (int c=0; c< aircraft.length; c++)
		{
			...
			if (capableAssignment != null && !aircraft[c].fitsAboard(capableAssignment))
				continue;
...
%>
				...
				<td><a class="link" href="<%= response.encodeURL(buyUrl) %>">Buy</a>
<%				for (int loop=0; loop < staffGroups.length; loop++) 
				{ 
%>					| <a class="link" href="<%= response.encodeURL(buyUrl + "&groupId=" + staffGroups[loop].groupId) %>">Buy for <%= staffGroups[loop].groupName %></a>
<% 				} 
%>
			</td>
			</tr>
