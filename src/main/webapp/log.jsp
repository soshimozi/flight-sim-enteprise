...
<%
	...
	if (logs.length > 0)
	{
		...
		double totalIncome = log.getIncome();
...
%>	
	...
	<tbody>
	<tr><td class="logHead"><%= Formatters.getUserTimeFormat(user).format(log.getTime())%></td><td class="logHead cost"><%= log.getFrom() + " &rarr; " + log.getTo()%> (<%=  flightTime %>)</td></tr>
	<tr><td class="space"></td><td></td></tr>
	<tr><td class="type">Income</td><td class="cost"><%= Formatters.currency.format(log.getIncome()) %></td></tr>
	<tr><td class="space"></td><td></td></tr>
	<tr><td class="type">Rental</td><td class="cost"></td></tr>
	<tr><td class="type indent">Amount</td><td class="cost"><%= rentalCost %></td></tr>
	<tr><td class="type indent">Cost per unit</td><td class="cost ul"><%= Formatters.currency.format(log.getRentalPrice()) %></td></tr>
	<tr><td class="type">Total rental cost</td><td class="cost total"><%= Formatters.currency.format(-log.getRentalCost()) %></td></tr>
	<tr><td class="space"></td><td></td></tr>
	<tr><td class="type">Additional cost</td><td class="cost"></td></tr>
	<tr><td class="type indent">Fuel</td><td class="cost"><%= Formatters.currency.format(-log.getFuelCost()) %></td></tr>
	<tr><td class="type indent">Landing Fee</td><td class="cost"><%= Formatters.currency.format(-log.getLandingCost()) %></td></tr>
	<tr>
	  <td class="type indent">Additional Crew </td>
	  <td class="cost"><%= Formatters.currency.format(-log.getCrewCost()) %></td></tr>
	<tr>
	...
	<tr><td class="type">Total additional cost</td><td class="cost total"><%= Formatters.currency.format(-log.getFuelCost()-log.getLandingCost()-log.getCrewCost()-log.getFboAssignmentFee()-log.getmptTax()) %></td></tr>
	<tr><td class="space"></td><td></td></tr>
	<tr><td class="type">Distance bonus</td><td class="cost total"><%= Formatters.currency.format(log.getBonus()) %></td></tr>	
	<tr class="total"><td class="type">Earnings this flight</td><td class="cost total"><%= Formatters.currency.format(total) %></td></tr>
<%	if (isGroup) { %>	
	<tr><td class="space"></td><td></td></tr>
	<tr><td class="type total">Paid to group "<%= groupName %>"</td><td class="cost total"><%= Formatters.currency.format(paidToGroup) %></td></tr>
	<tr>
	  <td class="type total">Paid to pilot</td>
	  <td class="cost total"><%= Formatters.currency.format(paidToPilot) %></td></tr>
	<tr><td class="space"></td><td></td></tr>
<% } %>
	</tbody>
	</table>
...
<% } %>
	...
	<td><a class="normal" href="<%= response.encodeURL("airport.jsp?icao=" + log.getFrom()) %>"><%= log.getFrom() %></a></td>
	<td><a class="normal" href="<%= response.encodeURL("airport.jsp?icao=" + log.getTo()) %>"><%= log.getTo() %></a></td>
	<td><a class="normal" href="<%= response.encodeURL("aircraftlog.jsp?registration=" + log.getAircraft()) %>"><%= log.getAircraft() %></a></td>
	<td><%= type %></td>
	<td><%= flightTime %></td>
	<td><%= log.getDistance() %></td>
	<td><%= Formatters.currency.format(total) %></td>
	<td><a class="link" href="<%= response.encodeURL("log.jsp?" + linkOptions + "from=" + from + "&id=" + log.getId()) %>">View</a></td>
	</tr>
<%
	}
%>
